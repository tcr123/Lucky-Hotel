...
import com.example.fop_hotel_gui.Table.A004ReviewTable;
...
public class A004Review extends database implements Initializable {
    ...
    @FXML
    public TableView<A004ReviewTable> A004ReviewTable;
    public TableColumn<A004ReviewTable, String> No;
    public TableColumn<A004ReviewTable, String> Review;
    public TableColumn<A004ReviewTable, String> Rating;

    private ObservableList<A004ReviewTable> TABLEROW = FXCollections.observableArrayList();


    @Override
    public void initialize(URL location, ResourceBundle resources) {
        // No.setCellValueFactory(new PropertyValueFactory<A001ReviewTable,String>("No"));
        Review.setCellValueFactory(new PropertyValueFactory<A004ReviewTable, String>("Review"));
        Rating.setCellValueFactory(new PropertyValueFactory<A004ReviewTable, String>("Rating"));

        showA004ReviewTable();
    }

    public void showA004ReviewTable(){
        TABLEROW.clear();
        Connection connection = getConnections();
        try {
            if(!connection.isClosed()){
                String sql = "SELECT " + "    `a004`.`Review`,\n" + "    `a004`.`Rating`\n" + "FROM `fopgui`.`a004`;";
                ...
                while (resultSet.next()){
                    ...
                    A004ReviewTable a004Table = new A004ReviewTable(Review,Rating);

                    TABLEROW.add(a004Table);
                }
                A004ReviewTable.getItems().setAll(TABLEROW);
            }
    ...
    public double UpdateReview() {
        ...
        try {
            if (!connection.isClosed()) {
                ...
                preparedStatement.setString(2, "A004");
                try {
    ...
    public void AddReview(ActionEvent actionEvent)throws IOException{
        Parent root= FXMLLoader.load(getClass().getResource("A004AddReview.fxml"));
