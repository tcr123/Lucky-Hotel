...
import com.example.fop_hotel_gui.Table.A001ReviewTable;
...
public class A001Review extends database implements Initializable {
    ...
    @FXML
    public TableView<A001ReviewTable> A001ReviewTable;
    public TableColumn<A001ReviewTable, String> Review;
    public TableColumn<A001ReviewTable, String> Rating;

    private ObservableList<A001ReviewTable> TABLEROW = FXCollections.observableArrayList();


    @Override
    public void initialize(URL location, ResourceBundle resources) {
        // No.setCellValueFactory(new PropertyValueFactory<A002ReviewTable,String>("No"));
        Review.setCellValueFactory(new PropertyValueFactory<A001ReviewTable, String>("Review"));
        Rating.setCellValueFactory(new PropertyValueFactory<A001ReviewTable, String>("Rating"));

        showA001ReviewTable();
    }


    public void showA001ReviewTable(){
        TABLEROW.clear();
        Connection connection = getConnections();
        try {
            if(!connection.isClosed()){
                String sql = "SELECT " + "    `a001`.`Review`,\n" + "    `a001`.`Rating`\n" + "FROM `fopgui`.`a001`;";
                ...
                while (resultSet.next()){
                    ...
                    A001ReviewTable a001Table = new A001ReviewTable(Review,Rating);

                    TABLEROW.add(a001Table);
                }
                A001ReviewTable.getItems().setAll(TABLEROW);
            }
    ...
    public double UpdateReview() {
        ...
        try {
            if (!connection.isClosed()) {
                ...
                preparedStatement.setString(2, "A001");
                try {
