...
import com.example.fop_hotel_gui.Table.A003ReviewTable;
...
public class A003Review extends database implements Initializable {
    ...
    @FXML
    public TableView<A003ReviewTable> A003ReviewTable;
    public TableColumn<A003ReviewTable, String> No;
    public TableColumn<A003ReviewTable, String> Review;
    public TableColumn<A003ReviewTable, String> Rating;

    private ObservableList<A003ReviewTable> TABLEROW = FXCollections.observableArrayList();


    @Override
    public void initialize(URL location, ResourceBundle resources) {
        // No.setCellValueFactory(new PropertyValueFactory<A001ReviewTable,String>("No"));
        Review.setCellValueFactory(new PropertyValueFactory<A003ReviewTable, String>("Review"));
        Rating.setCellValueFactory(new PropertyValueFactory<A003ReviewTable, String>("Rating"));

        showA003ReviewTable();
    }

    public void showA003ReviewTable(){
        TABLEROW.clear();
        Connection connection = getConnections();
        try {
            if(!connection.isClosed()){
                String sql = "SELECT " + "    `a003`.`Review`,\n" + "    `a003`.`Rating`\n" + "FROM `fopgui`.`a003`;";
                ...
                while (resultSet.next()){
                    ...
                    A003ReviewTable a003Table = new A003ReviewTable(Review,Rating);

                    TABLEROW.add(a003Table);
                }
                A003ReviewTable.getItems().setAll(TABLEROW);
            }
    ...
    public double UpdateReview() {
        ...
        try {
            if (!connection.isClosed()) {
                ...
                preparedStatement.setString(2, "A003");
                try {
    ...
    public void AddReview(ActionEvent actionEvent)throws IOException{
        Parent root= FXMLLoader.load(getClass().getResource("A003AddReview.fxml"));
